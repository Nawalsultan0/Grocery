            ...
            <form onSubmit={onSubmithandler}
             className="md:p-10 p-4 space-y-5 max-w-lg">
                        ...
                        Array(4).fill('').map((_, index) =>
                            ...
                            <label key={index} htmlFor={`image${index}`}>
                                ...
                                <input  type="file" id={`image${index}`}
                                 onChange={(e)=>{
                                    const updtaedfiles = [...Files];
                                    updtaedfiles[index] = e.target.files?.[0]
                                    setFiles(updtaedfiles);
                                }} hidden />
                                 <img src={Files[index] ? URL.createObjectURL(Files[index]) :
                                  assets.upload_area } 
                                  alt="Upload Area" className='max-w-24 cursor-pointer' />
                                </label>
                        )
                        )}
                    </div>
                ...
                <input onChange={(e)=>setname(e.target.value)} value={name}
                 id="product-name" tye="text" placeholder="Type here" 
